<?php
/*
Template Name: Natural Medicine
*/
?>
...
<section class="relative isolate overflow-hidden px-6 py-24 sm:py-32 lg:px-8">
    <?php $natural_medicine_header = get_field('natural_medicine_header'); ?>
    <?php if ($natural_medicine_header) : ?>
        <div class="absolute inset-0 -z-10 h-full w-full object-cover">
            <img src="<?php echo esc_url($natural_medicine_header['url']); ?>" alt="<?php echo esc_attr($natural_medicine_header['alt']); ?>" class="h-full w-full object-cover">
            <div class="absolute inset-0 bg-darkpink opacity-30"></div>
        </div>
    <?php endif; ?>
    <div class="mx-auto max-w-2xl text-center">
        <h1 class="text-5xl font-secondary font-bold tracking-tight text-whitesmoke sm:text-8xl"><?php the_field('natural_medicine_heading') ?></h1>
    </div>
</section>
    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div class="mx-auto max-w-2xl lg:text-center">
            <div class="mb-10 sm:mb-16 flex justify-center">
                <h2 class="text-5xl font-secondary font-bold tracking-tight text-lightpink sm:text-8xl opacity-50 absolute z-10 lg:top-16 md:top-16 top-14"><?php the_field('natural_medicine_subheading_1') ?></h2>
                <h3 class="mt-12 text-2xl font-primary font-bold tracking-tight text-black text-center sm:mt-10 sm:text-3xl relative z-20"><?php the_field('natural_medicine_subheading_2') ?></h3>
            </div>
        </div>
        ...
        <div class="mx-auto lg:max-w-none">
            <?php
            $args = array(
                'post_type'      => 'nmedicine-point',
                'posts_per_page' => -1,
                'order'          => 'ASC',
            );

            $nmedicine_points_query = new WP_Query($args);

            if ($nmedicine_points_query->have_posts()) :
            ?>
            <dl class="grid grid-cols-1 gap-x-8 gap-y-10 lg:max-w-none lg:grid-cols-3">
                <?php
                while ($nmedicine_points_query->have_posts()) : $nmedicine_points_query->the_post();
                ?>
                <div class="flex flex-col">
                    <h4 class="flex items-center gap-x-3 text-base font-semibold leading-7 text-gray-900">
                        ...
                        <?php the_field('natural_medicine_point_title') ?>
                    </h4>
                    <p class="mt-2 text-sm sm:text-base font-primary leading-7 tracking-tight text-black"><?php the_field('natural_medicine_point_description') ?></p>
                </div>
                <?php
                endwhile;
                wp_reset_postdata();
        ...
        <div class="mt-16 w-full border-solid border-[0.075rem] border-black shadow-sm">
            ...
            <div id="<?php echo esc_attr($tab_id); ?>Content">
                ...
                <div class="hidden p-4 bg-transparent" id="<?php echo esc_attr($tab_id); ?>-treatment" role="tabpanel" aria-labelledby="<?php echo esc_attr($tab_id); ?>-treatment-tab">
                    ...
                    <p class="mb-3 text-sm sm:text-base font-primary leading-7 tracking-tight text-black"><?php the_field('natural_medicine_treatment_description'); ?></p>
                </div>
